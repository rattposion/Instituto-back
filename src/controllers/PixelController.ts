...
export class PixelController {
  ...
  async getPixelById(req: AuthRequest, res: Response) {
    ...
    const { data: pixel, error } = await Database.query
      .from('pixels')
      .select('*')
      .eq('id', id)
      .eq('workspace_id', req.user!.workspaceId)
  ...
  async createPixel(req: AuthRequest, res: Response) {
    ...
    const { data: existingPixel } = await Database.query
      .from('pixels')
      .select('id')
      .eq('pixel_id', pixelId)
      .eq('workspace_id', req.user!.workspaceId)
  ...
  async updatePixel(req: AuthRequest, res: Response) {
    ...
    const { data: existingPixel, error: fetchError } = await Database.query
      .from('pixels')
      .select('*')
      .eq('id', id)
      .eq('workspace_id', req.user!.workspaceId)
  ...
  async deletePixel(req: AuthRequest, res: Response) {
    ...
    await Database.query.from('events').delete().eq('pixel_id', id);
    await Database.query.from('conversions').delete().eq('pixel_id', id);
    await Database.query.from('diagnostics').delete().eq('pixel_id', id);
  ...
  async getPixelAnalytics(req: AuthRequest, res: Response) {
    ...
    const { data: eventsData, error: eventsError } = await Database.query
      .from('events')
